...
public class RolesService implements RolesServiceBase {
    ...
    public RolesService(JpaRoleRepository roleRepository, JpaHumanUserPassportRepository passportRepository) {
    ...
    public void activateSuperAdmin(UUID userId, String activationKey) {
        var targetPassport = passportRepository.findById(userId).orElseThrow(EntityNotFoundException::new);
        if (!activationKeys.contains(activationKey)) throw new EntityNotFoundException("Activation Key not found");
    ...
    public void setUserAdmin(UUID userId) {
        ...
        var targetUser = passportRepository.findById(userId)
