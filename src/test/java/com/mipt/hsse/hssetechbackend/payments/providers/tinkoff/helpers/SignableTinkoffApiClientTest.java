...
class SignableTinkoffApiClientTest {
  ...
  void testRequestsWithoutSignMark() {
    ...
    tinkoffApiClient.get("test", entity);
    tinkoffApiClient.post("test", entity);
    tinkoffApiClient.delete("test", entity);
    tinkoffApiClient.put("test", entity);
    tinkoffApiClient.patch("test", entity);

    verify(requestsSigner, times(5)).createSign(eq(entity), eq(false));
    verify(apiClientBase, times(1)).get(eq("test"), eq(entity));
    verify(apiClientBase, times(1)).post(eq("test"), eq(entity));
    verify(apiClientBase, times(1)).delete(eq("test"), eq(entity));
    verify(apiClientBase, times(1)).put(eq("test"), eq(entity));
    verify(apiClientBase, times(1)).patch(eq("test"), eq(entity));
  }
